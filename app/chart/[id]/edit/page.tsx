...
function isChartEditDataEqual(a: ChartEditData | null, b: ChartEditData | null): boolean {
    ...
    const compareCategory = (ca: ChartEditData["leftCategory"], cb: ChartEditData["leftCategory"]) => {
    ...
    for (let i = 0; i < ac.length; i++) {
        ...
        if (x.reverseCompatibilityScore !== y.reverseCompatibilityScore) return false;
    }
...
export default function ChartEditPage() {
    ...
    const handleAddNode = (side: "left" | "right", name: string) => {
        ...
        setData((prev) => {
            ...
            const next = side === "left"
                ? { ...prev, leftCategory: { ...prev.leftCategory, elements: [...prev.leftCategory.elements, { id: tempId, name }] } }
                : { ...prev, rightCategory: { ...prev.rightCategory, elements: [...prev.rightCategory.elements, { id: tempId, name }] } };
            return next;
        });
    }

    // 相性データの保存処理
    const handleCompatibilitySave = (leftElementId: string, compatibilities: Array<{
    ...
    // ノード名の変更処理（左右両方に対応）
    const handleNodeNameChange = (elementId: string, newName: string, side: "left" | "right") => {
        setData((prev) => {
            if (!prev) return prev;
    ...
    const handleDeleteNode = (elementId: string, side: "left" | "right") => {
        setData((prev) => {
            ...
            const nextLeftElements = side === "left"
                ? prev.leftCategory.elements.filter(el => el.id !== elementId)
                : prev.leftCategory.elements;
            const nextRightElements = side === "right"
                ? prev.rightCategory.elements.filter(el => el.id !== elementId)
                : prev.rightCategory.elements;

            const nextCompatibilities = prev.compatibilities.filter(c => c.leftElementId !== elementId && c.rightElementId !== elementId);
    ...
    const handleSave = async () => {
        ...
        if (!data) return;
        // 各名前が入力されていなかったらリターン
        if (!data.title || !data.leftCategory.name || !data.rightCategory.name) return;
    ...
    return (<div className="flex flex-col gap-4">
        ...
        <div>
            {isChartEditDataEqual(initialDataRef.current, data)
                ? <Button variant="deactive" disabled>Save</Button> 
                : (<>
                    <Button 
                        className="mr-2" 
                        onClick={handleSave}
        ...
        <div>
            {!data.title && (
                <div className="text-red-600">Enter a title.</div>
            )}
        ...
        {/* chart名 */}
        <Label htmlFor="chart-title" className="text-primary">Title</Label>
        <Input 
            id="chart-title"
            type="text" 
            maxLength={100}
        ...
        <RadioGroup 
            ...
            onValueChange={(value: string) => setData({ ...data, isPublic: value === "public" })}
            className="mb-8"
        >
