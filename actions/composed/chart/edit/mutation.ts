...
export const saveChartData = async (
  ...
  try {
    ...
    if (leftCategoryError) {
      ...
      return {
        data: null,
        error: { message: 'Failed to update left category' },
      };
    }
    ...
    if (rightCategoryError) {
      ...
      return {
        data: null,
        error: { message: 'Failed to update right category' },
      };
    }

    // 3. 既存の要素を取得（削除対象の特定のため）
    const { data: existingLeftElements, error: leftElementsError } =
      await supabase
        .from('elements')
        .select('id')
    ...
    if (leftElementsError) {
      ...
      return {
        data: null,
        error: { message: 'Failed to fetch left elements' },
      };
    }

    const { data: existingRightElements, error: rightElementsError } =
      await supabase
        .from('elements')
        .select('id')
    ...
    if (rightElementsError) {
      ...
      return {
        data: null,
        error: { message: 'Failed to fetch right elements' },
      };
    }
    ...
    if (deletedLeftElementIds.length > 0) {
      ...
      if (deleteLeftError) {
        ...
        return {
          data: null,
          error: { message: 'Failed to delete left elements' },
        };
      }
    }
    ...
    if (deletedRightElementIds.length > 0) {
      ...
      if (deleteRightError) {
        ...
        return {
          data: null,
          error: { message: 'Failed to delete right elements' },
        };
      }
    }
    ...
    for (const element of chartData.leftCategory.elements) {
      if (element.id.startsWith('temp-')) {
        ...
        if (insertError) {
          ...
          return {
            data: null,
            error: { message: 'Failed to insert left element' },
          };
        }
        ...
        if (updateError) {
          ...
          return {
            data: null,
            error: { message: 'Failed to update left element' },
          };
        }
      }
    }
    ...
    for (const element of chartData.rightCategory.elements) {
      if (element.id.startsWith('temp-')) {
        ...
        if (insertError) {
          ...
          return {
            data: null,
            error: { message: 'Failed to insert right element' },
          };
        }
        ...
        if (updateError) {
          ...
          return {
            data: null,
            error: { message: 'Failed to update right element' },
          };
        }
      }
    }
    ...
    if (deleteCompatError) {
      ...
      return {
        data: null,
        error: { message: 'Failed to delete compatibilities' },
      };
    }
    ...
    if (chartData.compatibilities.length > 0) {
      ...
      const processedCompatibilities = chartData.compatibilities.map(
        (comp) => ({
          ...comp,
          leftElementId:
            tempIdToRealIdMap.get(comp.leftElementId) || comp.leftElementId,
          rightElementId:
            tempIdToRealIdMap.get(comp.rightElementId) || comp.rightElementId,
        })
      );

      // 有効なIDのみの相性データを処理
      const validCompatibilities = processedCompatibilities.filter(
        (comp) =>
      ...
      if (validCompatibilities.length > 0) {
        ...
        if (scoresError) {
          ...
          return {
            data: null,
            error: { message: 'Failed to fetch compatibility scores' },
          };
        }
        ...
        const compatibilitiesToInsert = validCompatibilities
          .filter(
            (comp) =>
              comp.compatibilityScore !== null &&
              comp.reverseCompatibilityScore !== null
          )
          .map((comp) => ({
            ...
            reverse_compatibility_score_id: scoreToIdMap.get(
              comp.reverseCompatibilityScore!
            )!,
            note: comp.note || null,
          }))
          .filter(
            (comp) =>
              comp.compatibility_score_id && comp.reverse_compatibility_score_id
          ); // 有効なIDのみ
        ...
        if (compatibilitiesToInsert.length > 0) {
          ...
          if (insertCompatError) {
            ...
            return {
              data: null,
              error: { message: 'Failed to insert compatibilities' },
            };
          }
        }
      } else {
