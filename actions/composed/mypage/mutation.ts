...
export async function duplicateChart({
  chartId,
}: DuplicateChartParams): Promise<Response<{ newChartId: string }>> {
  try {
    ...
    if (!result.success) {
      return {
        data: null,
        error: { message: result.error_message || 'Chart duplication failed' },
      };
    }
...
export async function deleteChart({
  chartId,
}: DeleteChartParams): Promise<Response<void>> {
  try {
    const supabase = await createClient();
    ...
    if (chartError || !chartData) {
      ...
      return {
        data: null,
        error: { message: 'Chart not found or access denied' },
      };
    }
