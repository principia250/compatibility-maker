-- チャート複製用のPostgreSQL関数を修正（IDの関係性を正しく修正）
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT NULL::UUID, FALSE, 'User not found in public schema'::TEXT;
        RETURN;
    END IF;
    ...
    IF NOT FOUND THEN
        -- チャートが存在するかチェック（権限なし）
        IF EXISTS (SELECT 1 FROM compatibility_charts WHERE id = p_chart_id) THEN
            RETURN QUERY SELECT NULL::UUID, FALSE, 'Access denied - chart exists but not owned by user'::TEXT;
        ELSE
            RETURN QUERY SELECT NULL::UUID, FALSE, 'Chart not found'::TEXT;
        END IF;
        RETURN;
    END IF;
