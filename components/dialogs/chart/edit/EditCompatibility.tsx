...
export const EditCompatibilityDialog = ({
    ...
    const [localCompatibilities, setLocalCompatibilities] = useState<Array<{
        rightElementId: string;
        compatibilityScore: number | null;
        reverseCompatibilityScore: number | null;
        note?: string;
    ...
    useEffect(() => {
        const initialCompatibilities = rightNodes.map(rightNode => {
            const existing = compatibilities.find(c => c.rightElementId === rightNode.id);
            return {
    ...
    const handleCompatibilityChange = (rightElementId: string, score: number | null) => {
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                        ...
                        <div className="flex-1 max-h-[500px] overflow-y-auto space-y-3 pr-2">
                            {rightNodes.map((rightNode) => {
                                const compatibility = localCompatibilities.find(c => c.rightElementId === rightNode.id);
